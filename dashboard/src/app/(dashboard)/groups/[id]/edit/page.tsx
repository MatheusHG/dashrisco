...
import {
  Lock, ArrowLeft, ArrowRight, Timer, Zap, Plus, X, Clock,
  Bell, MessageSquare, Check, Pencil, CheckCircle, Loader2, Save,
} from "lucide-react";
...
export default function EditGroupPage() {
  ...
  const fetchGroup = useCallback(async () => {
    try {
      const data = await api.fetch<GroupData>(`/groups/${groupId}`);
      setName(data.name);
      setLockSeconds(String(data.lockSeconds));
      setActive(data.active);
      setTriggerTypes((data.triggerTypes as string[]) || []);
      const filters = (data.triggerFilters as TriggerFilter[]) || [];
      setTriggerFilters(filters);
      setNoLimit(filters.length === 0);
      const slots = (data.timeSlots as TimeSlot[]) || [];
      setTimeSlots(slots);
      setUseTimeSlots(slots.length > 0);
  ...
  const addFilter = () => {
    setTriggerFilters([...triggerFilters, { field: "bet_value", operator: "GREATER", value: "", logicGate: null }]);
    setNoLimit(false);
  };
  ...
  const handleSave = async () => {
    if (triggerTypes.length === 0) { setError("Selecione pelo menos um tipo de aposta"); return; }
    setError(""); setSaving(true);
    try {
      await api.fetch(`/groups/${groupId}`, {
        method: "PUT",
        body: JSON.stringify({
          name, lockSeconds: Number(lockSeconds), active,
          notifyPanel, notifyChat,
          chatWebhookUrl: notifyChat ? chatWebhookUrl : null,
          triggerTypes,
          triggerFilters: noLimit ? [] : triggerFilters.map((f, i) => ({
    ...
    <div className="flex gap-8">
      <div className="flex-1 max-w-2xl space-y-6">
        <div className="flex items-center gap-3">
          <button onClick={() => router.push(`/groups/${groupId}`)} className="flex h-10 w-10 items-center justify-center rounded-xl border border-border/50 bg-card text-muted-foreground hover:bg-muted"><ArrowLeft className="h-4 w-4" /></button>
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-foreground">Editar Grupo</h1>
            <p className="text-sm text-muted-foreground">Passo {step} de 4 — {name}</p>
          </div>
        </div>

        <div className="flex gap-1">
          {STEP_LABELS.map((s, i) => {
            const num = i + 1; const Icon = s.icon; const act = num === step; const done = num < step;
            return (
              <button key={num} onClick={() => setStep(num)}
                className={`flex-1 flex items-center justify-center gap-1 py-2 rounded-lg text-[10px] font-medium transition-all
                  ${act ? "bg-primary text-primary-foreground" : done ? "bg-primary/15 text-primary cursor-pointer" : "bg-muted text-muted-foreground cursor-pointer"}`}>
              ...
              <div className="space-y-3 pt-2 border-t border-border/50">
                <label className={`flex cursor-pointer items-center gap-3 rounded-xl border p-4 transition-all ${useTimeSlots ? "border-primary bg-primary/5" : "border-border/50 hover:bg-muted/50"}`}>
                  <input type="checkbox" checked={useTimeSlots} onChange={(e) => {
                    ...
                    if (e.target.checked && timeSlots.length === 0) setTimeSlots([{ startHour: 0, endHour: 6, lockSeconds: 240 }, { startHour: 6, endHour: 24, lockSeconds: 120 }]);
            ...
            <CardContent className="p-6 space-y-5">
              ...
              {!noLimit && (
                <div className="space-y-3">
                  ...
                  <Button variant="outline" size="sm" className="rounded-xl gap-1.5" onClick={addFilter}><Plus className="h-3.5 w-3.5" /> Adicionar Condicao</Button>
            ...
            <CardContent className="p-5 space-y-3 text-sm">
              ...
              <div className="flex items-start justify-between group cursor-pointer rounded-lg p-2 -mx-2 hover:bg-muted" onClick={() => setStep(2)}>
                <div>
                  ...
                  {!noLimit && triggerFilters.length > 0 && <p className="text-[10px] font-mono text-muted-foreground mt-1">{triggerFilters.length} filtro(s)</p>}
