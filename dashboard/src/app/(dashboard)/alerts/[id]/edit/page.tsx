...
export default function EditAlertPage() {
  ...
  const fetchAlert = useCallback(async () => {
    ...
    try {
      ...
      const providers = Array.isArray(data.earlyPayoutProviders)
        ? (data.earlyPayoutProviders.filter((p): p is "NGX" | "RADAR" => p === "NGX" || p === "RADAR"))
        : [];
      setEarlyPayoutProviders(providers.length > 0 ? providers : ["NGX"]);
