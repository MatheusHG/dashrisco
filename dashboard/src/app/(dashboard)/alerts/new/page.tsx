...
export default function NewAlertPage() {
  ...
  const testQuery = async () => {
    ...
    setQueryTestLoading(true); setQueryTestError(""); setQueryTestResult(null);
    try {
      const result = await api.fetch<QueryTestResult>("/alerts/test-query", { method: "POST", body: JSON.stringify({ query: clickhouseQuery, sampleData: queryTestVars }) });
      setQueryTestResult(result);
    } catch (err: any) { setQueryTestError(err.message || "Erro ao executar query"); }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      router.push("/alerts");
    } catch (err) { setError(err instanceof Error ? err.message : "Erro ao criar alerta"); }
    ...
    <div className="flex gap-8">
      ...
      <div className="flex-1 max-w-2xl space-y-6">
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => router.push("/alerts")} className="flex h-10 w-10 items-center justify-center rounded-xl border border-border/50 bg-card text-muted-foreground hover:bg-muted"><ArrowLeft className="h-4 w-4" /></button>
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-foreground">Criar Alerta</h1>
        ...
        {step === 4 && (
          <Card className="rounded-2xl border-border/50 shadow-sm">
            <CardContent className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-2">
                {WEBHOOK_TYPES.map((t) => (
                  <button key={t.value} onClick={() => setWebhookType(t.value)}
              ...
              {queryEnabled && (
                <div className="space-y-5 pt-2">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {availableFields.map((f) => (
                        <button key={f.name} type="button" onClick={() => setClickhouseQuery((q) => q + `{{${f.name}}}`)}
                          className="text-[11px] font-mono bg-violet-500/10 text-violet-600 dark:text-violet-400 px-2 py-1 rounded-lg hover:bg-violet-500/20 transition-colors border border-violet-500/20">
                          {`{{${f.name}}}`}
                        </button>
                      ))}
                    </div>
                  ...
                  <div>
                    ...
                    <textarea value={clickhouseQuery} onChange={(e) => setClickhouseQuery(e.target.value)} rows={7}
                      placeholder={"SELECT count() AS deposit_count\nFROM majorsports.transfers\nWHERE user = '{{user_id}}'\n  AND type = 'DEPOSIT'\n  AND status = 'PAID'"}
                      className="w-full rounded-xl border border-border/60 bg-zinc-950/5 dark:bg-zinc-950/50 px-4 py-3 text-sm font-mono resize-y min-h-[140px] focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none leading-relaxed" />
                  </div>
            ...
            <CardContent className="p-6 space-y-5">
              ...
              <div className="flex items-center gap-3 pt-2">
                ...
                <Button size="sm" className="gap-1.5 rounded-xl" onClick={handleSubmit} disabled={loading}>
                  {loading ? <><Loader2 className="h-3.5 w-3.5 animate-spin" />Criando...</> : <><CheckCircle className="h-3.5 w-3.5" />Criar Alerta</>}
                </Button>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="rounded-2xl border-border/50 shadow-sm">
            <CardContent className="p-5 space-y-3 text-sm">
              ...
              <div className="flex items-start justify-between group cursor-pointer rounded-lg p-2 -mx-2 hover:bg-muted" onClick={() => setStep(7)}>
                <div>
                  <p className="text-[10px] uppercase tracking-wider text-muted-foreground flex items-center gap-1"><Database className="h-3 w-3" />Consulta DB</p>
                  {queryEnabled ? <Badge variant="secondary" className="text-[9px] mt-1">Habilitada{queryConditions.length > 0 ? ` (${queryConditions.length} condicoes)` : ""}</Badge> : <span className="text-muted-foreground italic text-xs">Desabilitada</span>}
                </div>
                <Pencil className="h-3 w-3 text-muted-foreground opacity-0 group-hover:opacity-100 mt-1" />
              </div>
            </CardContent>
